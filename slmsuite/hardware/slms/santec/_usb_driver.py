"""
USB backend driver for Santec SLMs via FTDI FT601.

This module provides :class:`._SantecUSBDriver`, a self-contained class that wraps
the binary USB protocol used by Santec LCOS SLMs (SLM-200, and likely SLM-210 and
...
class _SantecUSBDriver:
    ...
    Note
    ~~~~
    The memory slots of the protocol methods (:meth:`upload_image`,
    :meth:`display_slot`, :meth:`erase_slot`) are 1-indexed, matching the wire
    protocol. Only :meth:`write_frame`, which implements the zero-indexed slot
    convention of the public API, converts between the two.
    ...
    def __init__(
        ...
        """
        Initialize the driver. Does not open the device; call :meth:`open`.
        ...
        resolution : (int, int)
            ``(width, height)`` of the SLM in pixels. Used by :meth:`prime` to
            size the initial blank frame. Defaults to ``(1920, 1200)``.
        channel : int
        ...
        if _PyD3XX is None:
            raise RuntimeError(
                "PyD3XX not available; cannot use the Santec USB backend.\n" "Install it with:  pip install PyD3XX"
            )
    ...
    def __enter__(self) -> "_SantecUSBDriver":
        """
    ...
    def write_frame(self, frame: npt.NDArray[np.uint16] | int, index: int | None = None) -> None:
        """
        Write a phase pattern to a memory slot, or switch the displayed slot.

        Note
        ~~~~
        Slots are zero-indexed here, matching the public API convention, and are
        converted to the 1-indexed slots of the wire protocol.
    ...
    def get_status(self) -> tuple[int, str, str]:
        """
        Poll and decode the current FPGA status.

        The raw status string is available from the private :meth:`_poll_status`.

        Returns
        -------
        (int, str, str)
            ``(code, name, note)``, using the same convention as the DLL backend's
            ``SLM_STATUS_DICT``.
        """
        response = self._poll_status()
        return _STATUS_MAP.get(response, (-99, "SLM_UNKNOWN", response))
    ...
    def set_wavelength(self, wav_nm: int, max_phase_pi: float = 2.0) -> str:
        """
        ...
        max_phase_pi : float
            Maximum phase in units of pi, rounded to the nearest integer before
            being passed to the firmware. Defaults to 2 (= 2*pi).
        ...
        str
            FPGA response string.
        """
        return self._control_command(b"WL", [wav_nm, int(round(max_phase_pi))], longrunning=True)
