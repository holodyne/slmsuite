...
class FeedbackHologram(Hologram):
    ...
    def update_target(
        ...
        # Zero the undefined region. Boolean-mask assignment would route
        # through cupy's nonzero(), which synchronizes to read the selection
        # count; the elementwise select below is equivalent and stays on the
        # device.
        if null_region is None:
            # No null region of any kind: every undefined (nan) pixel becomes a hard zero.
            cp.copyto(self.target, 0, where=undefined)
        else:
            # Honor the null region (passed mask and/or circle) regardless of radius_frac.
            cp.copyto(self.target, 0, where=cp.logical_and(undefined, null_region))
